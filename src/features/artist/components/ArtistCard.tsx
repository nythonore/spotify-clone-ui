...
export const ArtistCard = ({ data }: ArtistCardPros) => {
	return (
		<Link
			to={`/artist/${data.id}`}
			className='hover-primary group min-w-[70%] flex-1 cursor-pointer rounded-lg bg-[#171717] py-2 shadow-lg hover:bg-[#302F2F] hover:bg-opacity-70 md:min-w-[30%] lg:p-4 xl:min-w-0'
		>
			<div>
				<img
					alt={data.name}
					src={data.image}
					className='h-40 w-full rounded-full object-cover'
				/>
